...
All assets original. Generate on the MacBook, rsync to the Studio. Full detail in `docs/ART-PIPELINE.md`.

1. **2D** (textures, sprites, skyboxes, UI, weapon icons, concept art) → Codex CLI image generation via
   `tools/artgen/gen.sh`, harvested from `~/.codex/generated_images/<session>/`.
2. **3D meshes** → headless Blender scripts in `tools/blender/`, textured with the above.
3. **Audio** → synthesized/procedural or CC0. No copyrighted tracks.
4. **Every asset** gets an entry in that game's `assets/manifest.json`: prompt, output path, sha256.
   Binaries stay gitignored; `tools/artgen/rebuild.sh` regenerates them from the manifest.
...
## Open decision — blocks Game 1

**Humanoid animation.** Codex generates images, not animation. STORMFALL and STRIKE PROTOCOL both need a
walk / run / crouch / aim / reload / death set on a humanoid rig, and hand-authoring that in Blender is days
of work for mediocre results. Options:

1. **Mixamo** — free, high quality, but requires a manual browser download (recommended).
2. **Procedural / IK-driven** — fully original, tends to look uncanny.
3. **Higgsfield `generate_3d`** — available via MCP, unproven for rigged characters.

Ask before committing to one. STORMFALL's character controller is blocked on this answer.
